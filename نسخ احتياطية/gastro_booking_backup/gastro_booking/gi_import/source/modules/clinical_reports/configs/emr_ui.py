...
from app.modules.clinical_reports.configs import emr_forms
...
PHASE_FORMS = {
    WF_CONTEXT: ("context", emr_forms.EmrContextForm, emr_forms.bind_context_form),
    WF_ACCESS: ("access", emr_forms.EmrAccessForm, emr_forms.bind_access_form),
    WF_IMAGING: ("lesion", emr_forms.EmrLesionForm, emr_forms.bind_lesion_form),
    WF_THERAPY: ("resection", emr_forms.EmrResectionForm, emr_forms.bind_emr_resection_form),
    WF_CLOSURE: ("closure", emr_forms.EmrClosureForm, emr_forms.bind_closure_form),
    WF_SYNTHESIS: ("synthesis", emr_forms.EmrSynthesisForm, emr_forms.bind_synthesis_form),
}

EDITABLE_PHASE_STATES = list(PHASE_FORMS.keys())
TIMELINE_FORM = emr_forms.EmrTimelineForm
...
def extract_phase_data(phase_key: str) -> dict:
    ...
    if phase_key == "resection":
        return {
            ...
            "resection_technique": request.form.get("resection_technique", ""),
            "snare_type": request.form.get("snare_type", ""),
            "en_bloc": request.form.get("en_bloc", ""),
            "piecemeal": request.form.get("piecemeal", ""),
...
def phase_template_context(report, document, phase_state, sp, form=None) -> dict:
    ...
    ctx = {
        ...
        "yes_no_choices": emr_forms.YES_NO_CHOICES,
