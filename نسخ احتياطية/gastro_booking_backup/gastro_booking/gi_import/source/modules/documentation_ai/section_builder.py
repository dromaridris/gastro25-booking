...
class SectionBuilder:
    ...
    def _history_presenting(self, ctx: dict, key: str) -> dict:
        ...
        if text:
            ...
            lines = [f"- {f.get('label', f.get('question_id', 'finding'))}: {f.get('value', '')}" for f in findings[:10]]
    ...
    def _past_history(self, ctx: dict, key: str) -> dict:
        return self._result(key, "[Past history not captured in available structured data — physician to complete]", ["Past medical history not in structured intake"], [])

    def _examination(self, ctx: dict, key: str) -> dict:
        return self._result(key, "[Examination findings not in structured data — physician to complete]", ["Physical examination not documented"], [])
    ...
    def _investigations(self, ctx: dict, key: str) -> dict:
        ...
        parts = []
        for lab in labs[:8]:
            parts.append(f"{lab.get('test_code')}: {lab.get('value')} {lab.get('unit') or ''} ({lab.get('abnormal_flag')})")
    ...
    def _procedures_section(self, ctx: dict, key: str) -> dict:
        ...
        content = "\n".join(f"- Procedure session {p.get('session_id')}: {p.get('outcome') or 'pending'}" for p in procs) or "[No procedures recorded]"
    ...
    def _follow_up(self, ctx: dict, key: str) -> dict:
        ...
        parts = []
        for p in plans[:2]:
            parts.append(f"- {p.get('related_condition')}: {p.get('recommended_interval_text') or p.get('reason', '')[:100]}")
    ...
    def _referral_reason(self, ctx: dict, key: str) -> dict:
        diagnoses = ctx.get("working_diagnoses") or []
        content = f"Referral for specialist assessment of {diagnoses[0]}." if diagnoses else "[Referral reason — physician to specify]"
    ...
    def _clinical_summary(self, ctx: dict, key: str) -> dict:
        ...
        return self._result(key, "\n\n".join(parts), [], [("clinical_intake", "intake"), ("clinical_assessment", "assessment")])
    ...
    def _since_last_visit(self, ctx: dict, key: str) -> dict:
        summaries = [
            s for s in ((ctx.get("patient_journey") or {}).get("follow_up_plans") or [])
        ]
        ...
        return self._result(key, content, [] if summaries or journey_ctx.get("timeline") else ["Limited follow-up history"], [("patient_journey", "context")])

    def _generic(self, ctx: dict, key: str) -> dict:
        return self._result(key, f"[Section '{key}' — physician to complete]", [f"No structured data mapped for section '{key}'"], [])
    ...
    def _detect_conflicts(ctx: dict) -> list[str]:
        ...
        accepted = {d.get("diagnosis_name") for d in assessment_decisions if d.get("physician_status") == "accepted"}
        less_likely = {u.get("diagnosis_name") for u in interpretation if u.get("update_direction") == "less_likely"}
        overlap = accepted & less_likely
        for name in overlap:
            conflicts.append(f"Interpretation suggests '{name}' less likely but physician accepted it in assessment")
        return conflicts
    ...
    def _result(
        ...
        is_complete = not missing and bool(content) and "[physician to complete" not in content.lower()
        return {
