...
def build_multi_symptom_hpi(
    *,
    ...
    Only the FIRST symptom paragraph uses the full "The patient presented
    with X..." opener. Every subsequent symptom paragraph uses a lighter
    "Regarding Y, ..." connective (see render_hpi_paragraph's
    sub_symptom flag) — otherwise the note repeats "The patient presented
    with" once per symptom, which reads as a robotic list rather than a
    single coherent narrative.
    """
    from gi_platform.narrative.prose import render_hpi_paragraph, _paragraph
    ...
    for i, sym in enumerate(symptoms):
        ...
        # Inject symptom-level onset/duration when Q&A did not already capture it.
        already_has_timeline = any(
            'onset' in f.code.lower()
            or 'duration' in f.code.lower()
            or 'when did' in f.prompt.lower()
            or 'how long' in f.prompt.lower()
            or 'duration' in f.prompt.lower()
            for f in sym_facts
        )
        if onset and not already_has_timeline:
            # Force proper "present for months" phrasing — never raw "Months".
            timed = duration_as_timeline_phrase(onset) or phrase_from_fact(
                'Symptom duration', onset, 'text', code='sym.duration',
            )
            if timed:
        ...
        # If onset was injected but phrase_from_fact path somehow missed it,
        # ensure the semantic doc still carries a clean timeline token.
        if onset and not doc.groups.get(GROUP_TIMELINE):
            timed = duration_as_timeline_phrase(onset)
            if timed:
                doc.add(GROUP_TIMELINE, timed)
        para = render_hpi_paragraph(doc, sub_symptom=(i > 0))
        if para:
            paragraphs.append(para)

    if shared_facts:
        shared_doc = build_semantic_document(chief_complaint='collateral history', facts=shared_facts)
        shared_para = render_hpi_paragraph(shared_doc, sub_symptom=bool(paragraphs))
        if shared_para:
