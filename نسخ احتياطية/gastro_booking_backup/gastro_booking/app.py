...
def login():
    ...
    if request.method == 'POST':
        ...
        if not row['is_approved']:
            ...
            return redirect(url_for('login', next=next_url))
        expires = row['role_expires_at'] if 'role_expires_at' in row.keys() else None
        if expires:
            try:
                if datetime.fromisoformat(expires) < datetime.utcnow():
                    flash('Account access has expired. Contact HOD or admin for renewal.', 'error')
                    return redirect(url_for('login', next=next_url))
            except ValueError:
                pass
        ...
        try:
            ...
            must_change = False
        if not must_change and check_password_hash(row['password_hash'], 'admin123'):
            must_change = True
            get_db().execute(
                'UPDATE user SET must_change_password = 1 WHERE id = ?', (row['id'],)
            )
            get_db().commit()
        if must_change:
            session['must_change_password'] = True
            flash('You must set a new password before continuing.', 'error')
            return redirect(url_for('change_password'))
    ...
    return render_template('login.html', next=next_url, promo_images=promo_images)
...
def approve_password_reset(request_id):
    ...
    flash(
        ...
        f'Share this one-time link (valid {PASSWORD_RESET_TOKEN_HOURS}h): {reset_url}',
        'success',
    )
    return redirect(url_for('admin_panel'))
