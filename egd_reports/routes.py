"""EGD routes — structured report aliases, follow-up, registry, patient overview."""
...
from egd_reports.registry import build_registry_export, get_registry_rows
...
def register_egd_routes(app, *, get_db, login_required, roles_required):
    """Registry, patient overview, and follow-up — report editor is /upper-gi/ via advanced_reports."""

    @app.route('/upper-gi/research-registry')
    @login_required
    @roles_required(*CAN_ACCESS)
    def upper_gi_research_registry():
        ...
        return render_template(
            'egd/research_registry.html',
            rows=rows, start=start_str, end=end_str, error=error,
        )

    @app.route('/upper-gi/research-registry/export')
    @login_required
    @roles_required(*CAN_ACCESS)
    def upper_gi_research_registry_export():
        start_str = request.args.get('start', '')
        end_str = request.args.get('end', '')
        if not start_str or not end_str:
            flash('Select a date range first.', 'error')
            return redirect(url_for('upper_gi_research_registry'))
        try:
            start_d = datetime.strptime(start_str, '%Y-%m-%d').date()
            end_d = datetime.strptime(end_str, '%Y-%m-%d').date()
        except ValueError:
        ...
        wb.save(buf)
        buf.seek(0)
        filename = f'egd_registry_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}.xlsx'
        return send_file(
            buf,
            as_attachment=True,
    ...
    @app.route('/patient-upper-gi-overview/<int:appointment_id>')
    @login_required
    @roles_required(*CAN_ACCESS)
    def patient_upper_gi_overview(appointment_id):
        db = get_db()
        ...
        if not appt or not appointment_matches_procedure(cfg, appt['procedure_type']):
            flash('Not an Upper GI / PEG appointment.', 'error')
            return redirect(url_for('dashboard'))
        mrn = (appt['mrn'] or '').strip()
        sessions_raw = session_service.list_sessions_for_types(
            db, ('upper_gi', 'peg_tube'), mrn, fallback_row=appt,
        )
        ...
        for s in sessions_raw:
            report = db.execute(
                'SELECT * FROM upper_gi_v2_report WHERE appointment_id = ?', (s['id'],)
            ).fetchone()
            ...
            if report:
                followups = db.execute(
                    'SELECT * FROM upper_gi_followup WHERE report_id = ? ORDER BY followup_date, id',
                    (report['id'],),
                ).fetchall()
        ...
        return render_template(
            ...
            followup_module='upper_gi',
        )

    # --- Follow-up CRUD (mirrors ERCP pattern) ---

    def _followup_row_json(row):
        return {k: row[k] for k in row.keys()}

    @app.route('/api/upper-gi/<int:report_id>/followups')
    @login_required
    @roles_required(*CAN_ACCESS)
    def upper_gi_followups_list(report_id):
        rows = get_db().execute(
            'SELECT * FROM upper_gi_followup WHERE report_id = ? ORDER BY followup_date, id',
            (report_id,),
        ).fetchall()
    ...
    @app.route('/api/upper-gi/<int:report_id>/followups', methods=['POST'])
    @login_required
    @roles_required(*CAN_ACCESS)
    def upper_gi_followup_create(report_id):
        db = get_db()
        ...
        db.commit()
        new_row = db.execute(
            'SELECT * FROM upper_gi_followup WHERE id = ?', (cur.lastrowid,)
        ).fetchone()
        return jsonify(_followup_row_json(new_row)), 201

    @app.route('/api/upper-gi-followup/<int:followup_id>', methods=['PUT'])
    @login_required
    @roles_required(*CAN_ACCESS)
    def upper_gi_followup_update(followup_id):
        db = get_db()
        row = db.execute(
            'SELECT * FROM upper_gi_followup WHERE id = ?', (followup_id,)
        ).fetchone()
        if not row:
    ...
    @app.route('/api/upper-gi-followup/<int:followup_id>', methods=['DELETE'])
    @login_required
    @roles_required(*CAN_ACCESS)
    def upper_gi_followup_delete(followup_id):
        db = get_db()
        row = db.execute(
            'SELECT * FROM upper_gi_followup WHERE id = ?', (followup_id,)
        ).fetchone()
        if not row:
            return jsonify({'error': 'Not found'}), 404
        db.execute('DELETE FROM upper_gi_followup WHERE id = ?', (followup_id,))
        db.commit()
