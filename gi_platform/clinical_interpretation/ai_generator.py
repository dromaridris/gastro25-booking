...
from gi_platform.clinical_interpretation.permissions import require_interpretation_use
...
class InterpretationAIGenerator:
    ...
    def generate(
        ...
        clinical_context: dict[str, Any], deterministic_findings: list[dict[str, Any]],
    ) -> dict[str, Any]:
        ...
        prompt_engine.register_blocks(
            ...
            [
                ...
                PromptBlock(
                    block_id='interpretation_instruction', category='task',
                    content='Interpret clinical data for physician review only. Do not confirm diagnosis.',
                ),
                PromptBlock(block_id='clinical_context', category='context',
                            content=json.dumps(clinical_context, indent=2)),
                PromptBlock(block_id='deterministic_findings', category='context',
        ...
        prompt_text = prompt_engine.build(PROMPT_CLINICAL_REASONING, context_payload=clinical_context)
        ...
        try:
            ...
            self.session_manager.complete_session(
                self.db, ai_session, response=provider_response, execution_duration_ms=duration_ms,
                prompt_text=prompt_text, store_prompt=cfg.log_prompts, store_response=cfg.log_responses,
                parsed_response_json=json.dumps(parsed.to_dict()),
            )
            log_event(
                self.db, action='clinical_interpretation.ai_generation',
                entity_type='clinical_ai_session', entity_id=ai_session.id, user_id=user_id,
                details={'session_uuid': ai_session.session_uuid},
            )
            return {
