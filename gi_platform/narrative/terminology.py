...
def normalize_term(text: str) -> str:
    ...
    if not t:
        return ''
    key = t.lower()
    # Exact whole-phrase match — always safe.
    for src, dst in TERM_MAP.items():
        if key == src.lower():
            return dst
    # Partial match is only safe when nothing trails *after* the matched
    # term — otherwise the qualifier gets orphaned mid-sentence, e.g.
    # "difficulty swallowing solids" -> "dysphagia solids" (a bug: the
    # word "solids" is meaningful and must not be stranded). If there is
    # trailing content, leave the original phrase alone rather than
    # producing a garbled substitution.
    for src, dst in sorted(TERM_MAP.items(), key=lambda x: -len(x[0])):
        ...
        after = t[m.end():].strip(' ,.')
        if after:
            continue  # trailing qualifier — do not truncate it away
        before = t[:m.start()].strip()
        t = (f"{before} {dst}".strip() if before else dst)
    return t.strip()
...
def extract_clinical_phrase(prompt: str, *, prefer_parenthetical: bool = True) -> str:
    ...
    if prefer_parenthetical:
        ...
        if m:
            ...
            # Only take the parenthetical alone when the text outside it is
            # boilerplate (e.g. "Onset (gradual/sudden)?"). If there is a
            # substantive clinical word before it — "Is the vomit bilious
            # (green)?" — that word ("bilious") must not be discarded in
            # favour of the parenthetical clarifier ("green"); combine them.
            outside = (text[:m.start()] + ' ' + text[m.end():]).strip()
            outside = re.sub(r'^(is|are|was|were|does|did|has|have)\s+(the\s+)?(patient\s+)?', '', outside, flags=re.I)
            outside = outside.strip(' ?').strip()
            if inner and len(inner) < 60:
                if len(outside.split()) <= 1:
                    inner_clean = clean_internal_codes(inner)
                    return normalize_term(inner_clean).lower() or inner_clean.lower()
                # substantive term outside the parens — keep both rather
                # than silently dropping one, e.g. "bilious vomiting
                # (green)?" -> "bilious vomiting (green)".
                outside_clean = normalize_term(clean_internal_codes(outside)) or outside
                inner_clean = clean_internal_codes(inner)
                return f"{outside_clean.lower()} ({inner_clean.lower()})"
...
# Bare unit answers from catalogue choices (Hours/Days/Weeks/Months/Years).
_DURATION_UNIT_RE = re.compile(
    r'^(a\s+few\s+|several\s+|many\s+|one\s+to\s+three\s+|more\s+than\s+three\s+)?'
...
def normalize_duration_phrase(value: str) -> str | None:
    """Turn Hours/Months/a few hours into a lowercase clinical duration unit."""
    raw = (value or '').strip()
    if not raw:
        return None
    # Strip accidental "present for " wrappers so callers stay idempotent.
    raw = re.sub(r'^(present\s+for|for)\s+', '', raw, flags=re.I).strip()
    label = normalize_choice(raw)
    if _DURATION_UNIT_RE.match(label) or label.lower() in _DURATION_VALUES:
        return label.lower()
    if _DURATION_UNIT_RE.match(raw) or raw.lower() in _DURATION_VALUES:
        return raw.lower()
    return None


def duration_as_timeline_phrase(value: str) -> str | None:
    """Semantic timeline token for Stage 2 — always 'present for <unit>'."""
    unit = normalize_duration_phrase(value)
    return f"present for {unit}" if unit else None
...
def phrase_from_fact(prompt: str, value: str, answer_type: str, *, code: str = '') -> str | None:
    ...
    # Bare duration units (Hours/Months/Years) — always timeline phrasing,
    # even when the prompt is a vague "Symptom onset" injection from
    # multi-symptom sessions (that path used to emit raw "Months").
    bare_duration = duration_as_timeline_phrase(v)
    if bare_duration and v_l not in _ONSET_PATTERN_VALUES:
        return bare_duration
    ...
    if '.duration' in code_l or 'how long' in prompt_l or 'duration' in prompt_l or 'when did' in prompt_l:
        return f"present for {normalize_choice(cleaned)}"
